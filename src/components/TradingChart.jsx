...
function TradingChart({ selectedSymbol, compareSymbols = [] }) {
  ...
  useEffect(() => {
    ...
    // --- SÉRIES DE COMPARAISON (% normalisé, échelle droite séparée) ---
    const compareSeriesMap = {};
    if (compareSymbols.length > 0) {
      // Échelle dédiée à la comparaison (% change), positionnée à droite
    ...
    fetch(`${API_URL}/api/prices?ticker=${encodeURIComponent(selectedSymbol)}&interval=${candleInterval}`)
      ...
      .then(data => {
        ...
        for (let i = 0; i < rawData.length; i++) {
          ...
          rawData[i].ma10 = calcMA(10); rawData[i].ma100 = calcMA(100); rawData[i].ma365 = calcMA(365);
          if (i >= 19) {
            const sma20 = calcMA(20);
            let sumSq = 0;
            for (let j = 0; j < 20; j++) sumSq += Math.pow(rawData[i - j].close - sma20, 2);
            const sd20 = Math.sqrt(sumSq / 20);
            rawData[i].bbUpper = sma20 + 2 * sd20; rawData[i].bbLower = sma20 - 2 * sd20;
          } else { rawData[i].bbUpper = null; rawData[i].bbLower = null; }
          if (i === 13) { let sum = 0; for (let j = 0; j <= 13; j++) sum += rawData[j].tr; rawData[i].atr = sum / 14; }
          else if (i > 13) rawData[i].atr = (rawData[i - 1].atr * 13 + rawData[i].tr) / 14;
          else rawData[i].atr = null;
        }
      ...
      .catch(err => console.error('Erreur TradingChart:', err));

    // --- FETCH ACTIFS DE COMPARAISON ---
    compareSymbols.forEach(sym => {
      fetch(`${API_URL}/api/prices?ticker=${encodeURIComponent(sym)}&interval=${candleInterval}`)
        ...
        .then(data => {
          ...
          compareDataRef.current[sym] = rawData;

          // Normalisation en % depuis le premier point
          const base = rawData[0].close;
          const normalized = rawData.map(d => ({ time: d.time, value: ((d.close - base) / base) * 100 }));
          compareSeriesMap[sym].setData(normalized);
        })
        .catch(err => console.error(`Erreur comparaison ${sym}:`, err));
    });
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginBottom: '15px', borderBottom: '1px solid #2B2B43', paddingBottom: '15px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: '5px', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <button style={filterBtnStyle(indicators.atr, '#e91e63')} onClick={() => toggleIndicator('atr')}>ATR</button>
          ...
          {/* Légende comparaison en temps réel */}
          {compareSymbols.length > 0 && (
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              {compareSymbols.map((sym, i) => {
                ...
                const changeColor = numVal === null ? '#8a919e' : numVal >= 0 ? '#26a69a' : '#ef5350';
                return (
                  <span key={sym} style={{ fontSize: '12px', fontWeight: 'bold' }}>
                    <span style={{ color }}>● {sym}</span>
                    {val && <span style={{ color: changeColor }}> {numVal >= 0 ? '+' : ''}{val}%</span>}
                  </span>
                );
              })}
            </div>
