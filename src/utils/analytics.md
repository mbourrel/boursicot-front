...
## Rôle
Encapsule PostHog : initialisation, identification utilisateur, opt-in RGPD, envoi d'événements.
...
- `optInTracking()` — passe de `memory` à `localStorage+cookie` (sur consentement explicite)
- `identifyUser(clerkUserId)` — réconcilie session anonyme avec compte Clerk, active la persistance

## Fonctionnement
Singleton `initialized` : `posthog.init` n'est appelé qu'une fois. Sans `VITE_POSTHOG_KEY`, toutes les fonctions sont silencieuses (pas de crash en dev local sans `.env.local`).
...
## Utilisé par
- `index.jsx` — `initAnalytics()` avant le render
- `App.jsx` — `identifyUser`, `captureEvent('signup_completed')`
- `App.jsx` (Dashboard) — `captureEvent('chart_mode_changed', ...)`
